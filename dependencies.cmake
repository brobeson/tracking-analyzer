...
function(_write_targets_to_json targets output)
  ...
  foreach(target IN LISTS targets)
    ...
    string(JSON targets_json SET "${targets_json}" ${i} "${object_json}")
    math(EXPR i "${i} + 1")
    # if(TARGET ${target})
    #   get_target_property(aliased_target ${target} ALIASED_TARGET)
    #   if(aliased_target)
    #     # _log(STATUS "Searching for alias of ${aliased_target}")
    #     _update_aliased_target("${targets_json}" "${aliased_target}")
    #   endif()
    # endif()
  endforeach()
...
function(_write_object_to_json object)
  set(object_json "{}")
  string(JSON object_json SET "${object_json}" "cmake_id" "\"${object}\"")
  _to_plantuml_id(${object} plantuml_id)
  string(JSON object_json SET "${object_json}" "plantuml_id" "\"${plantuml_id}\"")
  _split_namespace_and_target(${object} namespace name)
  if(namespace)
    string(JSON object_json SET "${object_json}" "namespace" "\"${namespace}\"")
  endif()
  if(name)
    string(JSON object_json SET "${object_json}" "name" "\"${name}\"")
  else()
  ...
  if(TARGET ${object})
    ...
    string(JSON object_json SET "${object_json}" "stereotype" "\"${target_type}\"")
...
function(_write_library_to_json library target_json)
  set(dependency_json "{}")
  string(JSON dependency_json SET "${dependency_json}" "id" "\"${library}\"")
...
function(_update_aliased_target json_array aliased_target)
  _log(STATUS "Searching for alias of ${aliased_target}")
  string(JSON n LENGTH "${json_array}")
  math(EXPR n "${n} - 1")
  foreach(i RANGE ${n})
    string(JSON id GET "${json_array}" ${i} "id")
    if(id STREQUAL aliased_target)
      _log(STATUS "Found ${id}")
      string(REGEX MATCH "([^:]+)::([^:]+)" unused "${object}")
      string(JSON json_array SET "${json_array}" ${i} "namespace" "\"${CMAKE_MATCH_1}\"")
      string(JSON json_array SET "${json_array}" ${i} "name" "\"${CMAKE_MATCH_2}\"")
      set()
      break()
    endif()
  endforeach()
...
function(_to_plantuml_id cmake_name output)
  string(REPLACE ":" "_" plantuml_id "${cmake_name}")
  string(REPLACE "-" "_" plantuml_id "${plantuml_id}")
  string(REPLACE "+" "p" plantuml_id "${plantuml_id}")
  set(${output} "${plantuml_id}" PARENT_SCOPE)
endfunction()

function(_split_namespace_and_target target namespace_out target_out)
  string(REGEX MATCH "([^:]+)::([^:]+)" unused "${object}")
  set(${namespace_out} "${CMAKE_MATCH_1}" PARENT_SCOPE)
  set(${target_out} "${CMAKE_MATCH_2}" PARENT_SCOPE)
endfunction()
...
function(_write_plantuml_file)
  ...
  # Write the targets to a PlantUML file
  string(JSON n LENGTH "${ct_JSON_DATABASE}")
  math(EXPR n "${n} - 1")
  foreach(i RANGE ${n})
    string(JSON namespace ERROR_VARIABLE no_namespace GET "${ct_JSON_DATABASE}" ${i} "namespace")
    if(NOT no_namespace)
      string(APPEND plantuml_string "frame \"${namespace}\" {\n")
    endif()
    string(JSON name GET "${ct_JSON_DATABASE}" ${i} "name")
    string(APPEND plantuml_string "[${name}]")
    string(JSON stereotype ERROR_VARIABLE no_stereotype GET "${ct_JSON_DATABASE}" ${i} "stereotype")
    if(NOT no_stereotype)
      string(APPEND plantuml_string " <<${stereotype}>>")
    endif()
    string(JSON plantuml_id GET "${ct_JSON_DATABASE}" ${i} "plantuml_id")
    string(APPEND plantuml_string " as ${plantuml_id}\n")
    if(NOT no_namespace)
      string(APPEND plantuml_string "}\n")
    endif()
    file(APPEND "${whole_project_file}" "${plantuml_string}")

    # # Get the component stereotype from the target type.
    # string(JSON stereotype ERROR_VARIABLE no_stereotype GET "${ct_JSON_DATABASE}" ${i} "type")
    # if(no_stereotype)
    #   set(stereotype "")
    # else()
    #   string(TOLOWER "${stereotype}" stereotype)
    #   string(REPLACE "_" " " stereotype "${stereotype}")
    #   set(stereotype "<<${stereotype}>>")
    # endif()
    # # Get the PlantUML ID.
    # string(JSON puml_id ERROR_VARIABLE no_puml_id GET "${ct_JSON_DATABASE}" ${i} "puml_id")
    # if(NOT no_puml_id)
    #   set(puml_id "as ${puml_id}")
    # endif()
    # # Get the package from the target namespace.
    # # string(JSON package ERROR_VARIABLE no_package GET "${ct_JSON_DATABASE}" ${i} "namespace")
    # # if(NOT no_package)
    # #   file(APPEND "${whole_project_file}" "frame \"${package}\" {\n")
    # # endif()
    # file(APPEND "${whole_project_file}" "[${name}] ${stereotype} ${puml_id}\n")
    # # if(NOT no_package)
    # #   file(APPEND "${whole_project_file}" "}\n")
    # # endif()
  endforeach()

  # Write dependencies to a PlantUML file
  foreach(i RANGE ${n})
    string(JSON dependencies ERROR_VARIABLE no_dependencies GET "${ct_JSON_DATABASE}" ${i} "dependencies")
    if(no_dependencies)
      continue()
    endif()
    string(JSON puml_id ERROR_VARIABLE no_puml_id GET "${ct_JSON_DATABASE}" ${i} "puml_id")
    if(no_puml_id)
      string(JSON puml_id GET "${ct_JSON_DATABASE}" ${i} "id")
    endif()
    # string(JSON name ERROR_VARIABLE error GET "${ct_JSON_DATABASE}" ${i} "name")
    # if(error)
    #   string(JSON name GET "${ct_JSON_DATABASE}" ${i} "id")
    # endif()
    # string(REPLACE "-" "_" name "${name}")
    string(JSON m LENGTH "${dependencies}")
    math(EXPR m "${m} - 1")
    foreach(j RANGE ${m})
      string(JSON dependency GET "${dependencies}" ${j})
      list(APPEND dependency_links "${puml_id} --> ${dependency}")
    endforeach()
  endforeach()
  list(REMOVE_DUPLICATES dependency_links)
  string(REPLACE ";" "\n" dependency_links "${dependency_links}")
  file(APPEND "${whole_project_file}" "${dependency_links}")
